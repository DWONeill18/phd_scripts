...
int main(int argc, char *argv[])
{
 ...
 /* declare variables */
  ...
  int gridz, gridcounter_x[gridx][gridy], tot_covered;
  float width_x, width_y, width_z, Lx, Ly, Lz, min_x, min_y, max_x, max_y, percentage;

  FILE *ipf; /* input file */
  FILE *file = fopen("grid_points.dat", "w");
  FILE *file2 = fopen("percentage.dat", "w");
		...
		{/*printf ("Water molecule found!\n");*/
			if (z[j]>Lz/2)
			{
				...
				for (k=0; k<gridy; k++)
				{
					...
					if ((x[j]>=min_x && x[j]<max_x) && (y[j]>=min_y && y[j]<max_y))
					{	
						fprintf (file,"%i %i\n", i, k);
						gridcounter_x[i][k] ++;
					}
...
/*	else{
		...
		for (k=0; k<gridy; k++)
		{
		if (gridcounter_x[i][k]>0)
			{
				tot_covered ++;
			}	
		}
		percentage = 100.0*tot_covered/(gridx*gridy);
		fprintf (file2, "%d	  %f \n", (t+1), percentage);
