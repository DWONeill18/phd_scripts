...
main(int argc, char *argv[])
{
  ...
  for (r=0; r<=r_max; r+=0.0005) {
    ...
    if (r < lower_limit) {
      ...
      gprime = X * ((6 * (pow(sigma,6)) * (pow(r,5)))/(pow(((pow(r,6)) + (alpha * (pow(sigma,6)))),2)));
      
      h = X * (pow(sigma,12))/(pow(((alpha * (pow(sigma,6))) + (pow(r,6))),2));
      hprime = X * -((12 * (pow(r,5)) * (pow(sigma,12)))/(pow(((pow(r,6)) + (alpha * (pow(sigma,6)))),3)));
