...
main(int argc, char *argv[])
{
  ...
  for (r=0; r<=r_max; r+=0.0005) {
    ...
    if (r < lower_limit) {
      ...
      fprime = (-1/(pow(r,2)));      
      // -1 * C
      g = X * (pow (sigma,6)) * (-1 / ((alpha * (pow (sigma,6))) + (pow (r,6))));
      gprime = X * (6 * pow (sigma,6) * pow (r,5))/(pow (((alpha * pow (sigma,6)) + pow (r,6)),2));
      
      h = X * (pow (sigma,12))/((pow (alpha,2)) * (pow (sigma,12)) + (alpha * (pow (sigma,6)) * (pow (r,6))) + (pow (r,12)));
      hprime =  -X * ((12 * alpha * pow (sigma,18) * pow (r,5)) + (12 * pow (sigma,12) * pow (r,11)))/(pow (((pow (alpha,2) * pow (sigma,12)) + (2 * alpha * pow (sigma,6) * pow (r,6)) + pow (r,12)),2));
